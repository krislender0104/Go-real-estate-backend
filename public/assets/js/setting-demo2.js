...
fn_toggle_customSidebar();

function fn_toggle_customSidebar() {
	var toggle_customSidebar = false,
	custom_open = 0;
	
	if(!toggle_customSidebar) {
		var toggle = $('.custom-template .custom-toggle');
		$(".custom-toggle").css({"display":"none"});
		$(".btn_find_translator").on("click", function () {
			var task_id;
			task_id = $(this).attr('task_id');
			$("#postProjectForm #taskId").attr("value", task_id);
		...
		toggle.on('click', (function(){
			if (custom_open == 1){
				...
				custom_open = 1;
				$(".custom-toggle").css({"display":"block"});
			}
		})
		);
